...
exports.getAvailabilityStats = getAvailabilityStats;
exports.getPhysicalStats = getPhysicalStats;
exports.getResponsablesStats = getResponsablesStats;
exports.getCloudStats = getCloudStats;
...
function toSortedArray(countByKey) {
...
function groupByWithCost(items, key, costKey) {
    const result = {};
    items.forEach(item => {
        const name = String(item[key] || 'No especificado');
        if (!result[name])
            result[name] = { count: 0, costo: 0 };
        result[name].count++;
...
async function getSecurityStats() {
    ...
    const cloudItems = await index_1.prisma.inventarioCloud.findMany();
    // Antivirus grouping
...
async function getPhysicalStats() {
    ...
    const parseGarantia = (g) => {
        if (!g || g === 'Sin Garantia' || g === 'n/a' || g.trim() === '')
            return null;
...
async function getResponsablesStats() {
    ...
    servidores.forEach(s => {
        ...
        if (s.estado === 'Mantenimiento')
            data.vmsMantenimiento++;
        if (s.pais && !data.paises.includes(s.pais))
            data.paises.push(s.pais);
    });
    ...
    equiposFisicos.forEach(e => {
        ...
        const data = allResponsables.get(resp);
        data.totalEquipos++;
        if (e.pais && !data.paises.includes(e.pais))
            data.paises.push(e.pais);
        if (e.categoria && !data.categorias.includes(e.categoria))
            data.categorias.push(e.categoria);
    });
    // Procesar cloud
...
async function getCloudStats() {
    ...
    // Linux vs Windows
    const linuxCount = cloudItems.filter(i => (i.sistemaOperativo || '').toLowerCase().includes('linux')).length;
    const windowsCount = cloudItems.filter(i => (i.sistemaOperativo || '').toLowerCase().includes('windows')).length;
