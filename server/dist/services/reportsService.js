...
exports.generateVMReport = generateVMReport;
exports.generatePhysicalReport = generatePhysicalReport;
exports.generateExecutiveSummary = generateExecutiveSummary;
exports.exportToExcel = exportToExcel;
exports.getEmailConfig = getEmailConfig;
exports.sendReportByEmail = sendReportByEmail;
...
function generateVMReport(servidores, tipo, filtro) {
    let data = [...servidores];
    // Aplicar filtros
    if (filtro) {
        switch (tipo) {
...
function generatePhysicalReport(equipos, tipo, filtro) {
    let data = [...equipos];
    if (filtro) {
        switch (tipo) {
            case 'categoria':
...
function generateExecutiveSummary(servidores, equiposFisicos) {
    ...
    return {
        ...
        porEstado: Object.entries(servidores.reduce((acc, s) => {
            acc[s.estado] = (acc[s.estado] || 0) + 1;
            return acc;
        }, {})).map(([estado, count]) => ({ estado, count })),
        porPais: Object.entries(servidores.reduce((acc, s) => {
...
async function getEmailConfig() {
    return index_1.prisma.emailConfig.findFirst({
        where: { activo: true }
    });
}
// Enviar informe por email
async function sendReportByEmail(email, data, reportType, filename) {
