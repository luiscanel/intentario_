...
router.get('/', async (req, res) => {
    try {
        const alertas = await index_1.prisma.alerta.findMany({
...
router.get('/contador', async (req, res) => {
    try {
        const count = await index_1.prisma.alerta.count({
            where: { leida: false }
        });
...
router.post('/', async (req, res) => {
    try {
        ...
        const alerta = await index_1.prisma.alerta.create({
...
router.put('/:id/leer', async (req, res) => {
    try {
        ...
        const alerta = await index_1.prisma.alerta.update({
...
router.put('/marcar-todas-leidas', async (req, res) => {
    try {
        const userEmail = req.user?.email;
        await index_1.prisma.alerta.updateMany({
            where: { leida: false },
            data: { leida: true, leidaPor: userEmail, leidaAt: new Date() }
        });
...
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await index_1.prisma.alerta.delete({ where: { id: parseInt(id) } });
...
router.delete('/limpiar/leidas', async (req, res) => {
    try {
        const result = await index_1.prisma.alerta.deleteMany({
            where: { leida: true }
        });
