...
function calcularProximoBackup(frecuencia: string, hora: number, minuto: number, diaSemana?: number, diaMes?: number): Date {
...
router.get('/', authMiddleware, async (req, res) => {
  try {
    const backups = await prisma.backupProgramado.findMany({
      include: {
        historiales: {
          orderBy: { inicio: 'desc' },
          take: 5
...
router.get('/estadisticas', authMiddleware, async (req, res) => {
  try {
    const [total, activos, ultimoMes] = await Promise.all([
      prisma.backupProgramado.count(),
      prisma.backupProgramado.count({ where: { activo: true } }),
      prisma.historialBackup.count({
        where: {
    ...
    const exitosos = await prisma.historialBackup.count({
      where: { estado: 'success' }
    })
    
    const fallidos = await prisma.historialBackup.count({
      where: { estado: 'failed' }
    })
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    const backup = await prisma.backupProgramado.findUnique({
...
router.post('/', authMiddleware, requireAdmin, validate(backupProgramadoSchema), async (req, res) => {
  try {
    ...
    const backup = await prisma.backupProgramado.create({
      data: {
        nombre,
        tipo: tipo || 'completo',
        frecuencia,
...
router.put('/:id', authMiddleware, requireAdmin, validate(backupProgramadoUpdateSchema), async (req, res) => {
  try {
    ...
    if (frecuencia || hora || minuto || diaSemana || diaMes) {
      const bk = await prisma.backupProgramado.findUnique({ where: { id: parseInt(id) } })
      proximoBackup = calcularProximoBackup(
        frecuencia || bk?.frecuencia || 'daily',
        hora ?? bk?.hora ?? 0,
        minuto ?? bk?.minuto ?? 0,
        diaSemana ?? bk?.diaSemana,
        diaMes ?? bk?.diaMes
      )
    }
    
    const backup = await prisma.backupProgramado.update({
...
router.delete('/:id', authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    await prisma.backupProgramado.delete({
      where: { id: parseInt(id) }
    })
...
router.post('/:id/ejecutar', authMiddleware, async (req, res) => {
  try {
    const { id } = req.params
    ...
    const proximoBackup = calcularProximoBackup(
      ...
      backup.hora,
      backup.minuto,
      backup.diaSemana || undefined,
      backup.diaMes || undefined
    )
    
    await prisma.backupProgramado.update({
      where: { id: backup.id },
      data: {
...
router.get('/:id/historial', authMiddleware, async (req, res) => {
  try {
    ...
    const historiales = await prisma.historialBackup.findMany({
