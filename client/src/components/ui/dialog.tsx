...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full p-2 bg-slate-100 text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/20">
        <X className="h-4 w-4" />
...
const DialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 text-left p-6 pb-0",
      className
    )}
    {...props}
  />
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 p-6 pt-4 bg-slate-50",
      className
    )}
    {...props}
  />
